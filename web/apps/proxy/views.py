...
async def _test_proxy_connection(proxy_url: str) -> dict:
    ...
    For SOCKS5/Tor proxies:
      - rdns=True on ProxyConnector handles remote hostname resolution (no DNS leaks)
      - socks5:// scheme is used (aiohttp_socks does NOT accept socks5h://)
    ...
    # Normalise to socks5:// - aiohttp_socks rejects socks5h:// as an invalid scheme.
    # Remote hostname resolution (what socks5h means in curl) is handled by rdns=True.
    proxy_url = _ensure_scheme(proxy_url, "socks5" if is_socks5 else "http")
