...
def _ensure_scheme(url: str, proxy_type: str) -> str:
    ...
    if not url:
        return url
    # socks5h:// is a curl-ism that aiohttp_socks does NOT accept.
    # Remote hostname resolution is handled by rdns=True on ProxyConnector.
    # Normalise any socks5h:// the user may have typed back to socks5://.
    ...
    if "://" not in url:
        ptype_scheme = "socks5" if proxy_type in ("socks5", "socks") else proxy_type
